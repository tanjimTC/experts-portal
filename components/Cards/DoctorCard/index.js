...
const Index = ({ img, designation, about }) => {
  const [open, setOpen] = useState(false);

  const onOpenModal = () => setOpen(true);
  const onCloseModal = () => setOpen(false);
  return (
    <div className="bg-white rounded-md overflow-hidden relative shadow-md m-2">
      <div>
        <img className="w-full" src={img} alt="Experts Title" />
      </div>
      <div className="p-4">
        <h2 className="text-2xl text-sectionBG">{designation}</h2>
        <div className="flex justify-between mt-4 mb-4 text-gray-500">
          ...
          <div className="flex items-center">
            ...
            <span className="ml-1 lg:text-xl">500</span>
          </div>
          <div className="flex items-center">
      ...
      <Modal open={open} onClose={onCloseModal} center>
        <AppointmentBook onCloseModal={onCloseModal} />
      </Modal>
    </div>
  );
};
